...
def get_blobs(img_mask):
  ...
  copy_image_mask = copy.copy(img_mask)
  ...
  for y in range (img_mask_height):
    for x in range (img_mask_width):
      if copy_image_mask[y,x] == 1:
        blobs_list.append(expand_nr(copy_image_mask, (y,x), []))
...
def get_blob_centroids(blobs_list):
  ...
  for i in range(0, len(blobs_list)):
    centroid = (np.mean(blobs_list[i], axis = 0))
    print(centroid)
